...
########################
# This code keeps the same settings as "save_data_and_cov_for_analysis.py" but saves data only for small scales of 30<r Mpc/h<80.""
######################################################
    ...
    for zbin in [1,3]:
        ...
        for ell in [0,2,4]:
            for (ell1_d, ell2_d, ELL_d) in [(0,0,0),(1,1,0),(2,0,2),(1,1,2)]:
                ...
                cov_temp = cov_temp.reshape(rbin_max_2pcf - rbin_min_2pcf, 13, 13)
        ...
        for (ell1, ell2, ELL) in [(0,0,0),(1,1,0),(2,0,2),(1,1,2)]:
            for ell_d in [0,2,4]:
                ...
                cov_temp = cov_temp[:, rbin_min_2pcf:rbin_max_2pcf]
                cov_temp = cov_temp.reshape(13,13, rbin_max_2pcf - rbin_min_2pcf)
                # IMPORTANT:  r1 >= r2 for zeta000, zeta110,and  zeta112. #
                if ell1 == ell2:
        ...
        for (ell1, ell2, ELL) in [(0,0,0),(1,1,0),(2,0,2),(1,1,2)]:
            for (ell1_d, ell2_d, ELL_d) in [(0,0,0),(1,1,0),(2,0,2),(1,1,2)]:
                ...
                cov_temp = cov_temp.reshape(13,13,13,13)
                # IMPORTANT:  r1 >= r2 for zeta000, zeta110,and  zeta112. #
                if ell1 == ell2:
                    cov_temp = np.array([cov_temp[i, j, :, :] for i in range(rbin_min_3pcf, rbin_max_3pcf) for j in range(rbin_min_3pcf, rbin_max_3pcf) if i>=j ])
                else:
        ...
        zeta000_galaxy = np.loadtxt("%s/zeta000_galaxy.dat" % (INPUT), usecols=(2,), unpack=True).reshape(13,13)
        zeta202_galaxy = np.loadtxt("%s/zeta202_galaxy.dat" % (INPUT), usecols=(2,), unpack=True).reshape(13,13)
        zeta110_galaxy = np.loadtxt("%s/zeta110_galaxy.dat" % (INPUT), usecols=(2,), unpack=True).reshape(13,13)
        zeta112_galaxy = np.loadtxt("%s/zeta112_galaxy.dat" % (INPUT), usecols=(2,), unpack=True).reshape(13,13)
        ...
        zeta000_galaxy_W1 = np.loadtxt("%s/zeta000_galaxy_W1.dat" % (INPUT), usecols=(2,), unpack=True).reshape(13,13)
        zeta202_galaxy_W1 = np.loadtxt("%s/zeta202_galaxy_W1.dat" % (INPUT), usecols=(2,), unpack=True).reshape(13,13)
        zeta110_galaxy_W1 = np.loadtxt("%s/zeta110_galaxy_W1.dat" % (INPUT), usecols=(2,), unpack=True).reshape(13,13)
        zeta112_galaxy_W1 = np.loadtxt("%s/zeta112_galaxy_W1.dat" % (INPUT), usecols=(2,), unpack=True).reshape(13,13)
        ...
        zeta000_galaxy_W2 = np.loadtxt("%s/zeta000_galaxy_W2.dat" % (INPUT), usecols=(2,), unpack=True).reshape(13,13)
        zeta202_galaxy_W2 = np.loadtxt("%s/zeta202_galaxy_W2.dat" % (INPUT), usecols=(2,), unpack=True).reshape(13,13)
        zeta110_galaxy_W2 = np.loadtxt("%s/zeta110_galaxy_W2.dat" % (INPUT), usecols=(2,), unpack=True).reshape(13,13)
        zeta112_galaxy_W2 = np.loadtxt("%s/zeta112_galaxy_W2.dat" % (INPUT), usecols=(2,), unpack=True).reshape(13,13)
        ...
        zeta000_galaxy_W3 = np.loadtxt("%s/zeta000_galaxy_W3.dat" % (INPUT), usecols=(2,), unpack=True).reshape(13,13)
        zeta202_galaxy_W3 = np.loadtxt("%s/zeta202_galaxy_W3.dat" % (INPUT), usecols=(2,), unpack=True).reshape(13,13)
        zeta110_galaxy_W3 = np.loadtxt("%s/zeta110_galaxy_W3.dat" % (INPUT), usecols=(2,), unpack=True).reshape(13,13)
        zeta112_galaxy_W3 = np.loadtxt("%s/zeta112_galaxy_W3.dat" % (INPUT), usecols=(2,), unpack=True).reshape(13,13)
        ...
        zeta000_mean = np.loadtxt("%s/zeta000_mean.dat" % (INPUT), usecols=(2,), unpack=True).reshape(13,13)
        zeta202_mean = np.loadtxt("%s/zeta202_mean.dat" % (INPUT), usecols=(2,), unpack=True).reshape(13,13)
        zeta110_mean = np.loadtxt("%s/zeta110_mean.dat" % (INPUT), usecols=(2,), unpack=True).reshape(13,13)
        zeta112_mean = np.loadtxt("%s/zeta112_mean.dat" % (INPUT), usecols=(2,), unpack=True).reshape(13,13)
        ...
        xi0_RR = xi_R["xi0"]
        xi2_RR = xi_R["xi2"]
        zeta000_RR = zeta_R["zeta000"].reshape(13,13, Rmax)
        zeta202_RR = zeta_R["zeta202"].reshape(13,13, Rmax)
        zeta110_RR = zeta_R["zeta110"].reshape(13,13, Rmax)
        zeta112_RR = zeta_R["zeta112"].reshape(13,13, Rmax)
        ...
        OUTPUT = "data_and_cov_for_small_scales_only"
        try:
            os.mkdir(OUTPUT)
        except:
            pass
